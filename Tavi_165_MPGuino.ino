//Naming convention: Tavi (original author ) + 165 (1.5+ compatible) + MPGuino = Tavi_165_MPGuino.ino
//monolithic for ease of editing and to avoid multiple header calls and collisions
//skybolt -- added output to support json readers such as https://github.com/skybolt/autoUpdate
//
...
#define useJsonOutput true //skybolt added to enable and call json out routine and supress normal serial out
...
#endif
#else
const uint8_t processorSpeed = 8; // processor speed in megahertz
#ifdef useLegacyLCD
...
//begin adding defs and Idx elements for json output. Next action: add to EEPROM params //LOC AGX5
...
//end adding Idx elements for json output. //END LOC AGX5
...
simpletx(format ( (currentIdx << dfBitShift) | tFuelUsed, 3 )); //current trip fuel used
simpletx("/"); 
simpletx(format ( (tankIdx << dfBitShift) | tFuelUsed, 3 )); //tank trip fuel used
simpletx("/"); 
simpletx(format ( (instantIdx << dfBitShift) | tFuelRate, 3 )); ///current rate of fuel burn in units/time (in legacy used to be instantGPH)    
simpletx("/"); 
//simpletx(intformat(instantrpm() * 1 )); //rpm to test latency only vs tachometer and LCD vs raspi indicator (expect 2x looptime), try and find a Tavi rpm reader function
simpletx(format( (instantIdx << dfBitShift) | tEngineSpeed, 0) ); 

simpletx("\n");
simpletx("\",\"subtitle\":\"");
//simpletx(format((parms[tankSizeIdx] - fuelReserveGalIdx) - tank.gallons() )  ); //this is old remaining
simpletx(format (pTankSizeIdx - (tankIdx << dfBitShift) | tFuelUsed, 3) ); //this cn get negative, needs if/then for zero. But remember
//we have unsigned ints, therefore test to see if remaining is greater than tank size. We can get fun with if/then: instead of 0.0 remaining, it can be "only fumes remaining"       

simpletx(" remaining, ");  //can get fun with if/then: instead of 0.0 remaining, it can be "only fumes remaining"       

//simpletx(format (fuelReserveGalIdx));
if (  pSizeTankSize - (tankIdx << dfBitShift) | tFuelUsed < pOffsetTankSize) { //if tank_size - fuel_used is less than fuel_reserve, then ...
  simpletx(format( pSizeTankSize - (tankIdx << dfBitShift) | tFuelUsed, 3  ));   //return tank_size - fuel used (indicates reserve remaining)
  } else {
  simpletx(format(pOffsetTankSize, 3)); //else, just return the reserve amount, my default is 2.2 gallons
  }
simpletx(" e-reserve");
simpletx(format((eocIdleTankIdx << dfBitShift) | tFuelUsed, 3));
simpletx(" ga used@idle, "); 
simpletx(format((eocIdleTankIdx << dfBitShift) | tDistance, 3));
...
//simpletx(format(pSizeTankSize, 3)); //largest, full tank size (e.g, 13.8 g)
//instrLdEEPROM, 0x02, pTankSizeIdx
//simpletx(format((instrLdEEPROM, 0x02, pTankSizeIdx), 3)); //largest, full tank size (e.g, 13.8 g)
//can't figure out how to call tank size, how about tRemainingFuel + tFuelUsed? 
int fullTank = (eocIdleTankIdx << dfBitShift) | tRemainingFuel + (eocIdleTankIdx << dfBitShift) | tFuelUsed; 
simpletx(format(fullTank, 3)); 
//(eocIdleTankIdx << dfBitShift) | tRemainingFuel + 
//(eocIdleTankIdx << dfBitShift) | tFuelUsed, 3) ); 
simpletx(","); 

int reserveFuel = 2200;  
simpletx(format(fullTank - reserveFuel, 3)); 
//simpletx(format(pSizeTankSize - pOffsetTankSize, 3));  //full tank less reserve (e.g.13.8g - 2.2g = 11.6g)
simpletx(","); 
//simpletx(format(pOffsetTankSize, 3));  //reserve amount (e.g. 2.2g)
simpletx(format(reserveFuel, 3)); 
simpletx("\n");
simpletx("],\"measures\":[");

//all app is unsigned int, so we can't go below zero. We have to check for negative by checking for huge positives. 
//goal is to show safe fuel remaining (tank_size - reserve - fuel_used) until zero, then show zero.   
/*if (pSizeTankSize - pOffsetTankSize - (tankIdx << dfBitShift) | tFuelUsed > pSizeTankSize) { //if tank_size - reserve - is greater than tank size, must be an unsigned error
  simpletx(format(0, 3)); //since remaining is greater than tank size, return zero
    } else {
  simpletx(format( pSizeTankSize - pOffsetTankSize - (tankIdx << dfBitShift) | tFuelUsed, 3  )   ); // (13.8 - 2.2) = 11.6 - 11.7 = -1, or 43232565642 error, unsinged?
}*/  
if ( ((tankIdx << dfBitShift) | tRemainingFuel) - reserveFuel > fullTank)  {
//if ((fullTank - reserveFuel) - (tankIdx << dfBitShift) | tFuelUsed > fullTank) {
  simpletx(format(0, 3)); //less than reseve, shorter bar at zero
} else {
  simpletx(format(( (tankIdx << dfBitShift) | tRemainingFuel) - reserveFuel, 3)); 
}
simpletx(",");
simpletx(format ( (tankIdx << dfBitShift) | tRemainingFuel, 3  )   ); //longer bar, simple remaining
simpletx("\n");
simpletx("],\"markers\":[");
simpletx(format( (instantIdx << dfBitShift) | tFuelUsed, 3 )); 
...
simpletx(format ( (currentIdx << dfBitShift) | tDistance, 3 )); //tank fuel used
simpletx("/"); 
simpletx(format ( (currentIdx << dfBitShift ) | tFuelUsed, 3 )); //trip fuel used
//simpletx(intformat(getRNG() - getDTE()));  //shorter line initially, miles traveled raw, flips to longer line
simpletx("\n");
simpletx("\",\"subtitle\":\"");
simpletx(format ( (tankIdx << dfBitShift) | tDistanceToEmpty, 3  ));  //this needs to be distance to BINGO. Need to make a function for this. Not fully empty, this is when your safe fuel runs out. Safe = tank_size - reserve
simpletx("mi to E,");


//number here is mpg * reserve, unless you have less than reserve left
//So if reserve is 2.2 and mpg is 30, it's 66. Once you go below reserve fuel, this counts down to zero. 
//so, if your tank_remaining is only 1.7 (less than reserve of 2.2), this number is 1.7 * 30 or 51. 
//Needs if/then since we can't call e-reserve gallons directly. 
//Maybe mpg * MIN(reserve, tFuelRemaining) is simpler. Certainly smaller. Will try that. 
simpletx(format( (tankIdx << dfBitShift) | tFuelEcon * MAX(pOffsetTankSize, (tankIdx << dfBitShift) | tRemainingFuel), 3) );//(mpg * reserve) vs (mpg * tFuelRemaining)
//simpletx(format( (tankIdx << dfBitShift) | tFuelEcon * (pSizeTankSize - pOffsetTankSize), 3));  //MPG * tank_size - tank_gal (this is safe max distance) 
simpletx(" mi e-reserve, "); 


simpletx(format( /*(tanksize - tankoffset) * tankMPG*/ pSizeTankSize - pOffsetTankSize * (tankIdx << dfBitShift) | tFuelEcon, 3)); //distance to empty - reserve
simpletx("mi safe range,"); 
simpletx(format((tankIdx << dfBitShift) | tDistanceToEmpty, 3)); //distance to a completely emtpy tank
simpletx("mi dry range");

simpletx("\n");
simpletx("\",\"ranges\":[");
simpletx(format(/*mpg * tank_size*/pSizeTankSize *   (tankIdx << dfBitShift) | tFuelEcon, 3) ); //range max - dry range
simpletx(","); 
simpletx(format(  (pSizeTankSize - pOffsetTankSize * (tankIdx << dfBitShift) | tFuelEcon) / 1, 3 )); //range 2, safe range
simpletx(","); 
simpletx(format(  (pSizeTankSize - pOffsetTankSize * (tankIdx << dfBitShift) | tFuelEcon) / 2, 3 )); //range 3, half of safe range

simpletx("\n");
simpletx("],\"measures\":[");
simpletx(format( (tankIdx << dfBitShift) | tFuelEcon * MAX(pOffsetTankSize, (tankIdx << dfBitShift) | tRemainingFuel), 3) );//(mpg * reserve) vs (mpg * tFuelRemaining). shows miles of e-reserve in bar form. Duplicate above line.

simpletx("\n");
simpletx("],\"markers\":[ ");
simpletx(format((tankIdx << dfBitShift) | tDistanceToEmpty, 3));//line is distance to empty
simpletx("]},");
...
simpletx(format(MIN(40000,  (instantIdx << dfBitShift) | tFuelEcon), 3 )    );
...
simpletx(format(MIN(40000,(currentIdx << dfBitShift) | tFuelEcon ), 3    ) );
simpletx(",");
simpletx(format(MIN(40000,(   tankIdx << dfBitShift) | tFuelEcon ), 3    ) );
simpletx(",");
simpletx(format(MIN(40000,(instantIdx << dfBitShift) | tFuelEcon ), 3    ) );
simpletx("],\"markers\":[");
simpletx(format(MIN(40000,(instantIdx << dfBitShift) | tFuelEcon ), 3    ) );
